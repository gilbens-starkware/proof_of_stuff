import "./chunk-FM7WUVZV.js";

// node_modules/get-starknet-core/dist/core.js
var U = Object.defineProperty;
var m = (I, M, e) => M in I ? U(I, M, { enumerable: true, configurable: true, writable: true, value: e }) : I[M] = e;
var L = (I, M, e) => (m(I, typeof M != "symbol" ? M + "" : M, e), e);
var y = (I, M, e) => {
  if (!M.has(I))
    throw TypeError("Cannot " + e);
};
var c = (I, M, e) => (y(I, M, "read from private field"), e ? e.call(I) : M.get(I));
var z = (I, M, e) => {
  if (M.has(I))
    throw TypeError("Cannot add the same private member more than once");
  M instanceof WeakSet ? M.add(I) : M.set(I, e);
};
var l = (I, M, e, N) => (y(I, M, "write to private field"), N ? N.call(I, e) : M.set(I, e), e);
var C = (I, M, e) => (y(I, M, "access private method"), e);
var Q = [
  {
    id: "argentX",
    name: "Argent X",
    icon: "data:image/svg+xml;base64,",
    downloads: {
      chrome: "https://chrome.google.com/webstore/detail/argent-x-starknet-wallet/dlcobpjiigpikoobohmabehhmhfoodbb",
      firefox: "https://addons.mozilla.org/en-US/firefox/addon/argent-x",
      edge: "https://microsoftedge.microsoft.com/addons/detail/argent-x/ajcicjlkibolbeaaagejfhnofogocgcj"
    }
  },
  {
    id: "braavos",
    name: "Braavos",
    icon: "data:image/svg+xml;base64,",
    downloads: {
      chrome: "https://chrome.google.com/webstore/detail/braavos-wallet/jnlgamecbpmbajjfhmmmlhejkemejdma",
      firefox: "https://addons.mozilla.org/en-US/firefox/addon/braavos-wallet",
      edge: "https://microsoftedge.microsoft.com/addons/detail/braavos-wallet/hkkpjehhcnhgefhbdcgfkeegglpjchdc"
    }
  }
];
var Y = () => `${Date.now()}-${Math.floor(Math.random() * (9e12 - 1)) + 1e12}`;
var w = (I) => {
  for (let M = I.length - 1; M > 0; M--) {
    const e = Math.floor(Math.random() * (M + 1));
    [I[M], I[e]] = [I[e], I[M]];
  }
  return I;
};
var r = (...I) => (M) => I.reduce((e, N) => e.then(N), Promise.resolve(M));
var A;
var i;
var u;
var n;
var S;
var a;
var s;
var f = class {
  constructor(M) {
    z(this, n);
    z(this, a);
    z(this, A, false);
    z(this, i, void 0);
    z(this, u, void 0);
    L(this, "value");
    l(this, u, M), C(this, a, s).call(this);
  }
  set(M) {
    return !c(this, A) && !C(this, a, s).call(this) ? false : (this.delete(), this.value = M, M && (l(this, i, `${c(this, u)}-${Y()}`), localStorage.setItem(c(this, i), M)), true);
  }
  get() {
    return C(this, n, S).call(this), this.value;
  }
  delete() {
    return !c(this, A) && !C(this, a, s).call(this) ? false : (this.value = null, c(this, i) && localStorage.removeItem(c(this, i)), true);
  }
};
A = /* @__PURE__ */ new WeakMap(), i = /* @__PURE__ */ new WeakMap(), u = /* @__PURE__ */ new WeakMap(), n = /* @__PURE__ */ new WeakSet(), S = function() {
  this.value && this.set(this.value);
}, a = /* @__PURE__ */ new WeakSet(), s = function() {
  try {
    !c(this, A) && typeof window < "u" && (l(this, i, Object.keys(localStorage).find(
      (M) => M.startsWith(c(this, u))
    )), l(this, A, true), c(this, i) && this.set(localStorage.getItem(c(this, i))));
  } catch (M) {
    console.warn(M);
  }
  return c(this, A);
};
var k = {
  ACCOUNTS: "accounts"
};
function o(I, M) {
  var e, N;
  if ((e = M == null ? void 0 : M.include) != null && e.length) {
    const D = new Set(M.include);
    return I.filter((g) => D.has(g.id));
  }
  if ((N = M == null ? void 0 : M.exclude) != null && N.length) {
    const D = new Set(M.exclude);
    return I.filter((g) => !D.has(g.id));
  }
  return I;
}
var x = async (I) => {
  const M = await Promise.all(
    I.map(
      (e) => e.request({ type: "wallet_getPermissions" }).then((N) => N.includes(k.ACCOUNTS)).catch(() => false)
    )
  );
  return I.filter((e, N) => M[N]);
};
var b = (I) => {
  try {
    return I && [
      "request",
      "on",
      "off",
      "version",
      "id",
      "name",
      "icon"
    ].every((M) => M in I);
  } catch {
  }
  return false;
};
function O(I, M) {
  return Object.values(
    Object.getOwnPropertyNames(I).reduce((e, N) => {
      if (N.startsWith("starknet")) {
        const D = I[N];
        M(D) && !e[D.id] && (e[D.id] = D);
      }
      return e;
    }, {})
  );
}
var d = (I, M) => {
  if (M && Array.isArray(M)) {
    I.sort((N, D) => M.indexOf(N.id) - M.indexOf(D.id));
    const e = I.length - M.length;
    return [
      ...I.slice(e),
      ...w(I.slice(0, e))
    ];
  } else
    return w(I);
};
var E = typeof window < "u" ? window : {};
var W = {
  windowObject: E,
  isWalletObject: b,
  storageFactoryImplementation: (I) => new f(I)
};
function v(I = {}) {
  const { storageFactoryImplementation: M, windowObject: e, isWalletObject: N } = {
    ...W,
    ...I
  }, D = M("gsw-last");
  return {
    getAvailableWallets: async (g = {}) => {
      const j = O(
        e,
        N
      );
      return r(
        (t) => o(t, g),
        (t) => d(t, g.sort)
      )(j);
    },
    getAuthorizedWallets: async (g = {}) => {
      const j = O(
        e,
        N
      );
      return r(
        (t) => x(t),
        (t) => o(t, g),
        (t) => d(t, g.sort)
      )(j);
    },
    getDiscoveryWallets: async (g = {}) => r(
      (j) => o(j, g),
      (j) => d(j, g.sort)
    )(Q),
    getLastConnectedWallet: async () => {
      const g = D.get(), t = O(e, N).find(
        (h) => h.id === g
      ), [T] = await x(
        t ? [t] : []
      );
      return T || (D.delete(), null);
    },
    enable: async (g, j) => {
      await g.request({
        type: "wallet_requestAccounts",
        params: {
          silent_mode: j == null ? void 0 : j.silent_mode
        }
      });
      const t = await g.request({
        type: "wallet_getPermissions"
      });
      if (!(t != null && t.includes(k.ACCOUNTS)))
        throw new Error("Failed to connect to wallet");
      return D.set(g.id), g;
    },
    disconnect: async ({ clearLastWallet: g } = {}) => {
      g && D.delete();
    }
  };
}
var G = v();
export {
  k as Permission,
  G as default,
  v as getStarknet,
  b as isWalletObj,
  O as scanObjectForWallets
};
//# sourceMappingURL=get-starknet-core.js.map
